...
export default class PhoneInput extends PureComponent {
  constructor(props) {
    ...
    this.state = {
      ...
      disabledPressFlag: props.disabledPressFlag || false,
    };
  }
  ...
  onChangeText = (formatted, extracted) => {
    this.setState({ number: extracted });
    ...
    if (onChangeFormattedText) {
      ...
      if (code) {
        onChangeFormattedText(extracted.length > 0 ? `+${code}${extracted}` : extracted);
      } else {
        onChangeFormattedText(extracted);
      }
    }
  };
  ...
  render() {
    ...
    const { modalVisible, code, countryCode, number, disabled, disabledPressFlag } = this.state;
    return (
      <CountryModalProvider>
        <View
          style={[
        ...
        >
          <TouchableOpacity
            ...
            disabled={disabled}
            activeOpacity={disabledPressFlag ? 1 : 0.7}
            onPress={() => {
              if (!disabledPressFlag) {
